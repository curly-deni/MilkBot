...
def _get_init_fields(
    cls: typing.Type[APIModel],
...
class APIModel(pydantic.BaseModel, abc.ABC):
    ...
    def __parse_galias(
        ...
        if cls.__model_init_fields__:
            ...
            if (
                set(values) & cls.__model_init_fields__
                and not set(values) & cls.__api_init_fields__
            ):
                return values
    ...
    def __parse_timezones(
        cls, values: typing.Dict[str, typing.Any]
    ) -> typing.Dict[str, typing.Any]:
        """Timezones are a pain to deal with so we at least allow a plain hour offset."""
        for name, field in cls.__fields__.items():
            if (
                isinstance(values.get(name), datetime.datetime)
                and values[name].tzinfo is None
            ):
