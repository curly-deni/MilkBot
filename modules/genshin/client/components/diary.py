...
from modules.genshin import paginators, types, utility
from modules.genshin.client import cache, manager, routes
from modules.genshin.client.components import base
from modules.genshin.models.genshin import diary as models
...
class DiaryPaginator(paginators.PagedPaginator[models.DiaryAction]):
    ...
    def __init__(
        self, getter: DiaryCallback, *, limit: typing.Optional[int] = None
    ) -> None:
    ...
    def data(self) -> models.BaseDiary:
        ...
        if self._data is None:
            raise RuntimeError(
                "At least one item must be fetched before data can be gotten."
            )
    ...
    async def request_ledger(
        ...
        **kwargs: typing.Any,
        ...
        url = (
            ...
            if detail
            else routes.INFO_LEDGER_URL.get_url(self.region)
        )
