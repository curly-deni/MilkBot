...
from modules.genshin import errors, types, utility
...
from modules.genshin.models.genshin import calculator as models
...
class CalculatorClient(base.BaseClient):
    ...
    async def request_calculator(
        ...
        **kwargs: typing.Any,
        ...
        return await self.request(
            url, method=method, params=params, data=data, **kwargs
        )
    ...
    async def _get_calculator_items(
        ...
        *,
        ...
        payload: typing.Dict[str, typing.Any] = dict(
            page=1, size=69420, is_all=is_all, **filters
        )
        ...
        if not any(filters.values()) and not sync:
            cache = client_cache.cache_key(
                "calculator", slug=slug, lang=lang or self.lang
            )

        try:
            data = await self.request_calculator(
                endpoint, lang=lang, data=payload, cache=cache
            )
            ...
            if not autoauth:
                raise errors.GenshinException(
                    e.response, "Calculator sync is not enabled"
                ) from e
    ...
    async def get_complete_artifact_set(
        self,
        artifact: types.IDOr[
            typing.Union[genshin_models.Artifact, genshin_models.CalculatorArtifact]
        ],
        *,
        lang: typing.Optional[str] = None,
    ) -> typing.Sequence[models.CalculatorArtifact]:
        ...
        data = await self.request_calculator(
            ...
            cache=client_cache.cache_key(
                "calculator", slug="set", artifact=int(artifact), lang=lang or self.lang
            ),
        )
    ...
    async def update_character_names(
        self, *, lang: typing.Optional[str] = None
    ) -> None:
        """Update stored db characters with the names from the calculator."""
        characters = await self.get_calculator_characters(
            lang=lang, include_traveler=True
        )
        ...
        for char in characters:
            ...
            dbchar = genshin_models.DBChar(
                ...
                icon,
                char.name,
                "" if "Player" in icon else char.element,
                char.rarity,
            )
