...
from modules.genshin.models.genshin import character as character_models
from modules.genshin.models.genshin import chronicle as models
...
class GenshinBattleChronicleClient(base.BaseBattleChronicleClient):
    ...
    async def get_genshin_characters(
        ...
        *,
        ...
        data = await self._request_genshin_record(
            "character", uid, lang=lang, method="POST"
        )
    ...
    async def get_genshin_spiral_abyss(
        ...
        *,
        ...
        data = await self._request_genshin_record(
            "spiralAbyss", uid, lang=lang, payload=payload
        )
    ...
    async def get_genshin_notes(
        ...
        *,
        ...
        try:
            data = await self._request_genshin_record(
                "dailyNote", uid, lang=lang, cache=False
            )
            ...
            if not autoauth:
                raise errors.GenshinException(
                    e.response, "Real-time notes are not enabled."
                ) from e
    ...
    async def get_full_genshin_user(
        ...
        *,
        ...
        return models.FullGenshinUserStats(
            **user.dict(), abyss=abyss, activities=activities
        )
