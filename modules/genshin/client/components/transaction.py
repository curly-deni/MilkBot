...
from modules.genshin.models.genshin import transaction as models
    ...
    async def _get_transaction_page(
        ...
        *,
        ...
        mi18n_task = asyncio.create_task(
            self._fetch_mi18n("inquiry", lang=lang or self.lang)
        )
        ...
        for trans in data["list"]:
            ...
            transactions.append(
                model(**trans, kind=kind, reason_lang=lang or self.lang)
            )
    ...
    def transaction_log(
        ...
        *,
        ...
        return paginators.MergedPaginator(
            iterators, key=lambda trans: trans.time.timestamp()
        )
