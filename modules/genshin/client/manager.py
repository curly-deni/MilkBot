...
def parse_cookie(cookie: typing.Optional[CookieOrHeader]) -> typing.Dict[str, str]:
    ...
    return {
        str(k): v.value if isinstance(v, http.cookies.Morsel) else str(v)
...
class BaseCookieManager(abc.ABC):
    ...
    @classmethod
    def from_cookies(
        cls, cookies: typing.Optional[AnyCookieOrHeader] = None
    ) -> BaseCookieManager:
    ...
    def from_browser_cookies(
        cls, browser: typing.Optional[str] = None
    ) -> CookieManager:
        ...
        **kwargs: typing.Any,
        ...
        async with self.create_session() as session:
            async with session.request(
                method, str_or_url, proxy=self.proxy, **kwargs
            ) as response:
...
class CookieManager(BaseCookieManager):
    ...
    def set_browser_cookies(
        self, browser: typing.Optional[str] = None
    ) -> typing.Mapping[str, str]:
...
class CookieSequence(typing.Sequence[typing.Mapping[str, str]]):
    ...
    def __init__(
        self, cookies: typing.Optional[typing.Sequence[CookieOrHeader]] = None
    ) -> None:
    ...
    @cookies.setter
    def cookies(
        self, cookies: typing.Optional[typing.Sequence[CookieOrHeader]]
    ) -> None:
        if not cookies:
            self._cookies = []
            return
    ...
    def _sort_cookies(self) -> None:
        ...
        self._cookies.sort(
            key=lambda x: 0 if x[1] >= self.MAX_USES else x[1], reverse=True
        )
        ...
        **kwargs: typing.Any,
        ...
        for index, (cookie, uses) in enumerate(self._cookies._cookies.copy()):
            try:
                ...
                self._cookies._cookies[index] = (
                    cookie,
                    1 if uses >= self._cookies.MAX_USES else uses + 1,
                )
                return data
...
class InternationalCookieManager(BaseCookieManager):
    ...
    def __init__(
        ...
        cookies: typing.Optional[
            typing.Mapping[str, MaybeSequence[CookieOrHeader]]
        ] = None,
    ) -> None:
        self.set_cookies(cookies)
        ...
        **kwargs: typing.Any,
        ...
        for index, (cookie, uses) in enumerate(self._cookies[region]._cookies.copy()):
            try:
                ...
                self._cookies[region]._cookies[index] = (
                    cookie,
                    self._cookies[region].MAX_USES,
                )
            else:
...
def no_multi(func: CallableT) -> CallableT:
    ...
    def wrapper(
        ...
        if self.cookie_manager.multi:
            raise RuntimeError(
                f"Cannot use {func.__name__} with multi-cookie managers - data is private."
            )
