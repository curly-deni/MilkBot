...
def validate_uid(uid: str) -> bool:
    ...
    """
    return (
        len(uid) == 9 and uid.isdigit() and re.match(r"([1,2,5-9])\d{8}", uid)
    )  # noqa: E501
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            response = await session.request(
                "GET", _url, headers={**get_default_header(), **headers}
            )  # noqa: E501

            if response.status >= 400:
                LOGGER.warning(
                    f"Failure to fetch {_url} ({response.status}) Retry {retry} / {RETRY_MAX}"
                )  # noqa: E501
                retry += 1
                if retry > RETRY_MAX:
        ...
        return {"status": response.status, "content": json.loads(data)}
