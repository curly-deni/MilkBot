...
class CharacterInfo(BaseModel):
    ...
    def __init__(__pydantic_self__, **data: Any) -> None:
        ...
        # Get prop map
        __pydantic_self__.xp = (
            int(data["propMap"]["1001"]["ival"]) if "1001" in data["propMap"] else 0
        )  # noqa: E501
        __pydantic_self__.ascension = (
            int(data["propMap"]["1002"]["ival"]) if "1002" in data["propMap"] else 0
        )  # noqa: E501
        __pydantic_self__.level = (
            int(data["propMap"]["4001"]["ival"]) if "4001" in data["propMap"] else 0
        )  # noqa: E501

        # Constellation unlocked count
        __pydantic_self__.constellations_unlocked = (
            len(data["talentIdList"]) if "talentIdList" in data else 0
        )

        # Get max character level
        __pydantic_self__.max_level = (
            (__pydantic_self__.ascension * 10)
            + (10 if __pydantic_self__.ascension > 0 else 0)
            + 20
        )  # noqa: E501
        ...
        for constellation in character.constellations:
            ...
            __pydantic_self__.constellations.append(
                CharacterConstellations(
                    id=int(constellation),
                    name=_name,
                    icon=_constellation.icon,
                    unlocked=int(constellation) in data["talentIdList"]
        ...
        for skill in character.skills:
            ...
            __pydantic_self__.skills.append(
                CharacterSkill(
                    id=skill,
                    name=_name,
                    icon=_skill.icon,
