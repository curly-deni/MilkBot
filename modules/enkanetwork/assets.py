...
class Assets:
    ...
    def constellations(
        cls, id: int
    ) -> Union[assets.CharacterConstellationsAsset, None]:  # noqa: E501
        ...
        return assets.CharacterConstellationsAsset.parse_obj(
            {"id": id, **data, "icon": cls.create_icon_path(data["icon"])}
        )
    ...
    def skills(cls, id: int) -> Union[assets.CharacterSkillAsset, None]:
        ...
        return assets.CharacterSkillAsset.parse_obj(
            {"id": id, **data, "skillIcon": cls.create_icon_path(data["skillIcon"])}
        )
    ...
    def get_hash_map(cls, hash_id: str) -> Union[str, None]:
        LOGGER.debug(
            f"Getting nameTextMapHash {hash_id} with language: {cls.LANGS}"
        )  # noqa: E501
        for key in cls.HASH_MAP:
            if str(hash_id) in cls.HASH_MAP[key]:
                val = cls.HASH_MAP[key][str(hash_id)][cls.LANGS]
                LOGGER.debug(
                    f"Got nameTextMapHash {hash_id} with language: {key} (Value '{val}')"
                )  # noqa: E501
                return val

        LOGGER.error(
            f"nameTextMapHash {hash_id} not found with language: {cls.LANGS}"
        )  # noqa: E501
        return
    ...
    def create_character_icon(path: str) -> assets.CharacterIconAsset:
        return assets.CharacterIconAsset(
            ...
            banner=create_ui_path(
                path.replace("AvatarIcon_Side", "Gacha_AvatarImg")
            ),  # noqa: E501
        )
    ...
    def _set_language(cls, lang: Language) -> None:
        ...
        if lang is None or not lang.split("-")[0].lower() in list(Language):
            raise ValueError(
                "Language not supported. Please check your language."
            )  # noqa: E501
    ...
    def __load_assets_lang(cls) -> None:
        ...
        for FILENAME in FILE_LANG:
            ...
            cls.HASH_MAP[FILENAME.split(".")[0]] = json.load(
                cls.__load(os.path.join(_PATH, FILENAME))
            )  # noqa: E501
    ...
    def __load_assets_data(cls) -> None:
        ...
        for FILENAME in FILE_DATA:
            ...
            cls.DATA[FILENAME.split(".")[0]] = json.load(
                cls.__load(os.path.join(_PATH, FILENAME))
            )  # noqa: E501
