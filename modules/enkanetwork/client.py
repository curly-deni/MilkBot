...
class EnkaNetworkAPI:
    ...
    def __init__(
        self, lang: str = "en", debug: bool = False, key: str = ""
    ) -> None:  # noqa: E501
        # Logging
        logging.getLogger("enkanetwork").setLevel(
    ...
    async def fetch_user(self, uid: Union[str, int]) -> EnkaNetworkResponse:
        ...
        resp = await request(
            url=create_path(
                f"u/{uid}/__data.json" + ("?key={key}" if self.__key else "")
            )
        )  # noqa: E501
    ...
    async def update_assets(self) -> None:
        ...
        for folder in _PATH:
            for filename in os.listdir(_PATH[folder]):
                ...
                _json = await request(
                    url=self.RAWDATA.format(
                        PATH=f"master/exports/{folder}/{filename}"
                    )  # noqa: E501
                )

                self.LOGGER.debug(f"Writing {folder} file {filename}...")
                with open(
                    os.path.join(_PATH[folder], filename), "w", encoding="utf-8"
                ) as f:  # noqa: E501
                    json.dump(
                        _json["content"], f, ensure_ascii=False, indent=4
                    )  # noqa: E501
