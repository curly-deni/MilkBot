...
ICON_BASE = (
    "https://upload-os-bbs.mihoyo.com/game_record/honkai3rd/global/SpriteOutput/"
)
...
class Battlesuit(APIModel, Unique):
    ...
    def __autocomplete_figpath(
        cls, tall_icon: str, values: typing.Dict[str, typing.Any]
    ) -> str:
        ...
        if tall_icon:
            ...
            if values["id"] not in BATTLESUIT_IDENTIFIERS:
                BATTLESUIT_IDENTIFIERS[values["id"]] = tall_icon.split("/")[-1].split(
                    "."
                )[0]
    ...
    def _type_cn(self) -> str:
        ...
        return (
            match[1] if match else "ShengWu"
        )  # random default just so images keep working
