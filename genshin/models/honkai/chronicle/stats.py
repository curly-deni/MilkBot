...
class OldAbyssStats(APIModel):
    ...
    @pydantic.validator(
        "raw_q_singularis_rank", "raw_dirac_sea_rank", "raw_latest_rank", pre=True
    )
    def __normalize_rank(
        cls, rank: str
    ) -> typing.Optional[int]:  # modes.OldAbyss.__normalize_rank
        if "Unknown" in rank:
            return None
...
class HonkaiStats(APIModel):
    ...
    abyss: typing.Union[SuperstringAbyssStats, OldAbyssStats] = Aliased(
        mi18n="bbs/explain_text_1"
    )
    memorial_arena: MemorialArenaStats = Aliased(
        mi18n="bbs/battle_field_ranking_percentage"
    )
...
class FullHonkaiUserStats(HonkaiUserStats):
    ...
    def abyss_superstring(self) -> typing.Sequence[modes.SuperstringAbyss]:
        ...
        return [
            entry for entry in self.abyss if isinstance(entry, modes.SuperstringAbyss)
        ]
    ...
    def abyss_q_singularis(self) -> typing.Sequence[modes.OldAbyss]:
        ...
        return [
            ...
            if isinstance(entry, modes.OldAbyss) and entry.type == "Q-Singularis"
        ]
    ...
    def abyss_dirac_sea(self) -> typing.Sequence[modes.OldAbyss]:
        ...
        return [
            entry
            for entry in self.abyss
            if isinstance(entry, modes.OldAbyss) and entry.type == "Dirac Sea"
        ]
