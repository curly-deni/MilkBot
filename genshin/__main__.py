...
def client_command(
    ...
    @click.option(
        "--cookies", help="Cookie header to use in authentication.", default=None
    )
...
async def genshin_stats(client: genshin.Client, uid: int) -> None:
    ...
    for area in data.explorations:
        ...
        offerings = ", ".join(
            f"{o.name} {click.style(str(o.level), bold=True)}" for o in area.offerings
        )
...
async def genshin_characters(client: genshin.Client, uid: int) -> None:
    ...
    for char in characters:
        ...
        click.echo(
            f"lvl {char.level} C{char.constellation}, friendship lvl {char.friendship}"
        )
        click.echo(
...
async def genshin_notes(client: genshin.Client, uid: typing.Optional[int]) -> None:
    ...
    click.echo(
        f"{click.style('Resin:', bold=True)} {data.current_resin}/{data.max_resin}"
    )
    click.echo(
        f"{click.style('Realm currency:', bold=True)} {data.current_realm_currency}/{data.max_realm_currency}"
    )
    click.echo(
        f"{click.style('Commissions:', bold=True)} "
        f"{data.completed_commissions}/{data.max_commissions}",
        nl=False,
    )
    if (
        data.completed_commissions == data.max_commissions
        and not data.claimed_commission_reward
    ):
    ...
    if (
        data.transformer_recovery_time
        and data.transformer_recovery_time
        > datetime.datetime.now(datetime.timezone.utc)
    ):
        click.echo(
            f"{click.style('Transformer recovery:', bold=True)} {data.transformer_recovery_time}"
        )
    ...
    for expedition in data.expeditions:
        if expedition.remaining_time > datetime.timedelta(0):
            ...
            click.echo(
                f" - {expedition.status} | {remaining} - {expedition.character.name}"
            )
        else:
...
async def wishes(client: genshin.Client, limit: typing.Optional[int] = None) -> None:
    ...
    async for wish in client.wish_history(limit=limit):
        ...
        click.echo(
            f"{banner} | {wish.time.astimezone()} - {wish.name} ({'★' * wish.rarity} {wish.type})"
        )
...
async def banner_ids(client: genshin.Client) -> None:
    ...
    for banner in await client.get_banner_details(ids):
        click.echo(
            f"{banner.banner_id} - {banner.banner_type_name} ({banner.banner_type})"
        )
...
async def login(account: str, password: str, port: int) -> None:
    ...
    click.echo(
        f"Your cookies are: {click.style(base.output(header='', sep=';'), bold=True)}"
    )
