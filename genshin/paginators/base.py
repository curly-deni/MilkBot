...
class Paginator(typing.Generic[T], abc.ABC):
    ...
    def __repr__(self) -> str:
        kwargs = ", ".join(
            f"{name}={getattr(self, name, 'undefined')!r}"
    ...
    def _complete(self) -> typing.NoReturn:
        ...
        raise StopAsyncIteration(
            "No more items exist in this paginator. It has been exhausted."
        ) from None
...
class BasicPaginator(typing.Generic[T], Paginator[T], abc.ABC):
    ...
    def __init__(
        self, iterable: typing.Union[typing.Iterable[T], typing.AsyncIterable[T]]
    ) -> None:
...
class MergedPaginator(typing.Generic[T], Paginator[T]):
    ...
    async def _prepare(self) -> None:
        ...
        for order, (it, value) in enumerate(zip(self.iterators, first_values)):
            ...
            heapq.heappush(
                self._heap, self._create_heap_item(value, iterator=it, order=order)
            )
    ...
    async def __anext__(self) -> T:
        ...
        heapq.heapreplace(
            self._heap, self._create_heap_item(new_value, iterator=it, order=order)
        )
